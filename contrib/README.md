...
A Linux bash script that will set up traffic control (tc) to limit the outgoing bandwidth for connections to the Bitcoin Light network. This means one can have an always-on bitcoinlightd instance running, and another local bitcoinlightd/bitcoinlight-qt instance which connects to this node and receives blocks from it.
...
Contains files used to package bitcoinlightd/bitcoinlight-qt
