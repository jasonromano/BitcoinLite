...
The following new options are available for bitcoinlightd and bitcoinlight-qt:
...
The following rpc commands are available:
 ...
 - _keepass init_: sets up the association between bitcoinlightd and keepass by generating an AES key and sending an association message to KeePassHttp. This will trigger KeePass to ask for an Id for the association. Returns the association and the base64 encoded string for the AES key.
...
Sample initialization flow from _bitcoinlight-qt_ console (this needs to be done only once to set up the association):

 - Have KeePass running with an open database
 - Start _bitcoinlight-qt_
 - Open console
 - Type "_keepass init_" in bitcoinlight-qt console
 ...
 - Edit _bitcoinlight.conf_ and fill in these values
```
keepass=1
