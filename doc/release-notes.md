...
installer (on Windows) or just copy over /Applications/Bitcoinlight-qt (on Mac) or
bitcoinlightd/bitcoinlight-qt (on Linux).
...
All transaction fees are reduced 10x (from 10K per Kb to 1K per Kb), including fees for InstantSend (from 0.001 BTL per input to 0.0001 per input)
...
See `Help -> Command-line options` in Qt wallet or `bitcoinlightd --help` for more info.
...
- [`9901cf433`](https://github.com/dashpay/dash/commit/9901cf433) Fix for bitcoinlight-qt issue with startup and multiple monitors. (#1461)
...
Bitcoin Light tree 0.11.x was a fork of Bitcoin Core tree 0.9, Darkcoin was rebranded
to Dash.

Bitcoin Light tree 0.12.0.x was a fork of Bitcoin Core tree 0.10.

Bitcoin Light tree 0.12.1.x was a fork of Bitcoin Core tree 0.12.
