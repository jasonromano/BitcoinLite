...
bool AppInit(int argc, char* argv[])
{
    ...
    // Parameters
    //
    // If Qt is used, parameters/bitcoinlight.conf are parsed in qt/dash.cpp's main()
    ParseParameters(argc, argv);

    // Process help and version before taking care about datadir
    if (mapArgs.count("-?") || mapArgs.count("-h") ||  mapArgs.count("-help") || mapArgs.count("-version"))
    {
        std::string strUsage = _("Bitcoin Light Daemon") + " " + _("version") + " " + FormatFullVersion() + "\n";
        ...
        {
            strUsage += "\n" + _("Usage:") + "\n" +
                  "  bitcoinlightd [options]                     " + _("Start Bitcoin Light Daemon") + "\n";
    ...
    try
    {
        ...
        if (fCommandLine)
        {
            fprintf(stderr, "Error: There is no RPC client functionality in bitcoinlightd anymore. Use the bitcoinlight-cli utility instead.\n");
            exit(EXIT_FAILURE);
        }
#ifndef WIN32
        ...
        if (fDaemon)
        {
            fprintf(stdout, "Bitcoin Light server starting\n");
...
int main(int argc, char* argv[])
{
    ...
    // Connect bitcoinlightd signal handlers
    noui_connect();
