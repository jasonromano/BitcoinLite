...
unsigned int GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock, const Consensus::Params& params)
{
    ...
    if (nHeightNext > params.BTLHeight) retarget = DIFF_DGW; // After the fork we use Dark Gravity Well 3
    else if (nHeightNext == params.BTLHeight) retarget = DIFF_BTL_FORK; // The forking block - lowest difficulty
