...
const char * const BITCOIN_CONF_FILENAME = "bitcoinlight.conf";
const char * const BITCOIN_PID_FILENAME = "bitcoinlightd.pid";
...
boost::filesystem::path GetDefaultDataDir()
{
    ...
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\BitcoinLight
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\BitcoinLight
    // Mac: ~/Library/Application Support/BitcoinLight
    // Unix: ~/.bitcoinlight
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "BitcoinLight";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/BitcoinLight";
#else
    // Unix
    return pathRet / ".bitcoinlight";
#endif
#endif
}
                    ...
                    map<string, vector<string> >& mapMultiSettingsRet)
{
    ...
    if (!streamConfig.good()){
        // Create empty bitcoinlight.conf if it does not excist
    ...
    {
        // Don't overwrite existing settings so command line settings override bitcoinlight.conf
