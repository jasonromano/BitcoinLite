...
#if defined(NDEBUG)
# error "Bitcoin Light cannot be compiled without assertions."
#endif
...
bool static IsBTLHardForkEnabled(const CChainParams& chainParams, int nHeight) {
    return nHeight >= chainParams.GetConsensus().BTLHeight;
}

bool IsBTLHardForkEnabled(const CChainParams& chainParams, const CBlockIndex *pindexPrev) {
    if (pindexPrev == nullptr)
        return false;

    return IsBTLHardForkEnabled(chainParams, pindexPrev->nHeight);
}

bool IsBTLHardForkEnabledForCurrentBlock(const CChainParams& chainParams) {
    AssertLockHeld(cs_main);
    return IsBTLHardForkEnabled(chainParams, chainActive.Tip());
}
...
bool CheckTxInputs(const CTransaction& tx, CValidationState& state, const CCoinsViewCache& inputs, int nSpendHeight)
{
        ...
        for (unsigned int i = 0; i < tx.vin.size(); i++)
        {
            ...
            if (coins->IsCoinBase()) {
                if (nSpendHeight - coins->nHeight < COINBASE_MATURITY && coins->nHeight != ::Params().GetConsensus().BTLHeight)
                    return state.Invalid(false,
...
//    }
    ...
    if (IsBTLHardForkEnabled(Params(), pindex->pprev)) {
    ...
    if (nCurrentBlockHeight > chainparams.GetConsensus().BTLHeight) {
        ...
        // END DASH
    } else if (nCurrentBlockHeight == chainparams.GetConsensus().BTLHeight) {
        // Fork block
        CAmount blockReward = nFees + chainparams.GetConsensus().BTLInitialBudget;
        std::string strError = "";
        if (!IsBlockValueValid(block, pindex->nHeight, blockReward, strError))
            return state.DoS(0, error("ConnectBlock(BTL FORK BLOCK): coinbase pays too much (actual=%d vs limit=%d)", block.vtx[0].GetValueOut(), blockReward), REJECT_INVALID, "bad-cb-amount");

        const CScript *script = &block.vtx[0].vout[0].scriptPubKey;
        if (!block.vtx[0].IsCoinBase())
            return state.DoS(0, error("ConnectBlock(BTL FORK BLOCK): invalid coinbase transaction"));
        if (block.vtx[0].vout.size() != 1)
            return state.DoS(0, error("ConnectBlock(BTL FORK BLOCK): wrong number of coinbase outputs"));
        if (script->size() != 35)
            return state.DoS(0, error("ConnectBlock(BTL FORK BLOCK): invalid coinbase scriptPubKey"));
        if ((*script)[34] != OP_CHECKSIG)
            return state.DoS(0, error("ConnectBlock(BTL FORK BLOCK): invalid coinbase scriptPubKey"));

        CTxDestination txDestination;
        if (!ExtractDestination(*script, txDestination)) {
            return state.DoS(0, error("ConnectBlock(BTL FORK BLOCK): invalid destination"));
        }

        // Bitcoin Light Foundation gets to mine the first block after the fork
        if (CBitcoinAddress(txDestination).ToString() != "1JSp1M1EK7hKxkc53eQxCBP6d7FMM4LkSM") {
            return state.DoS(0, error("ConnectBlock(BTL FORK BLOCK): invalid coinbase address"));
        }
...
bool ContextualCheckBlockHeader(const CBlockHeader& block, CValidationState& state, CBlockIndex * const pindexPrev)
{
    ...
    // Check proof of work (took from Bitcoin Core source, because DGW is not applicable here)
    if (block.nBits != GetNextWorkRequired(pindexPrev, &block, consensusParams))
        return state.DoS(100, false, REJECT_INVALID, "bad-diffbits", false, strprintf("incorrect proof of work // height %d // block.nBits %d // GetNextWorkRequired %d", nHeight, block.nBits, GetNextWorkRequired(pindexPrev, &block, consensusParams)));
